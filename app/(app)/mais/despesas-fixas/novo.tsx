...
function valoresIniciais(): DespesaFixaFormValues {
  return {
    escopo: 'casa',
    casaId: null,
    categoriaId: null,
...
export default function NovaDespesaFixaScreen() {
  const { user } = useAuth();
  ...
  // despesa fixa" mostraria o que foi digitado na última vez.
  useFocusEffect(
    useCallback(() => {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      await createDespesaFixa(despesaFixaParaInput(values, Number(user.id)));
      router.back();
    } catch (e: unknown) {
