...
import { getReceitasFixas } from '@/src/services/api/receitasFixas';
...
export default function ContratosReceitasScreen() {
  const [filtro, setFiltro] = useState<Filtro>('vigentes');
  const [itens, setItens] = useState<ReceitaFixa[]>([]);
  ...
  const carregar = useCallback(() => {
    setLoading(true);
    setError(null);
    getReceitasFixas(filtro === 'todas' ? undefined : { vigente: filtro === 'vigentes' })
      .then(setItens)
  ...
  function abrirDetalhe(item: ReceitaFixa) {
    router.push({
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.vazio}>Nenhuma receita fixa encontrada.</Text>}
          renderItem={({ item }: { item: ReceitaFixa }) => {
                ...
                <View style={styles.itemInfo}>
                  ...
                  <Text style={styles.itemDetalhe}>
                    {[
                      ...
                      `dia ${item.dia_esperado_recebimento}`,
      ...
      <Pressable style={styles.botaoNovo} onPress={() => router.push('/(app)/mais/receitas-fixas/novo')}>
        <Text style={styles.botaoNovoTexto}>+ Nova receita fixa</Text>
      </Pressable>
    </View>
  );
}
