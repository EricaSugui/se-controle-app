...
import { createExcecaoReceitaFixa } from '@/src/services/api/receitasFixas';
import { CurrencyInput } from '@/src/components/ui/CurrencyInput';
import { notificar } from '@/src/utils/confirmar';
import { formatCurrency } from '@/src/utils/formatters';

export default function JustificarReceitaFixaScreen() {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      await createExcecaoReceitaFixa(Number(params.receitaFixaId), {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.info}>
          ...
          {'\n'}Valor esperado:{' '}
          {params.valorEsperado ? formatCurrency(Number(params.valorEsperado)) : 'variável'}
        </Text>
        ...
        <TextInput
          ...
          placeholder="Ex.: cliente cancelou o mês"
          multiline
          numberOfLines={3}
          maxLength={255}
        />
